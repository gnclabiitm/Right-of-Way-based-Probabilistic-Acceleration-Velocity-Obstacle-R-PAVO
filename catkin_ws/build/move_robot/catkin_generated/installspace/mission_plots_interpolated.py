...
#     plt.subplot(2,3,plot_params.index(param)+1) #to get single plot for each, comment this and uncomment the top line plt.figure() 
...
#         # Create a dictionary to store accumulated velocities for each unique time
#         accumulated_values = {}
#         for time, value in zip(time_data, values):
#             if time not in accumulated_values:
#                 accumulated_values[time] = []
#             accumulated_values[time].append(value)

#         # Calculate the average velocity for each unique time
#         unique_time = []
#         average_values = []
#         for time, value in accumulated_values.items():
#             unique_time.append(time)
#             average_values.append(np.mean(value))

#         # Interpolation function
#         interpolation_function = interp1d(unique_time, average_values, kind='cubic')
#         # Generate time points for smooth curve
#         smooth_time = np.linspace(min(unique_time), max(unique_time), 1000)
#         # Interpolate values for smooth curve
#         smooth_values = interpolation_function(smooth_time)

#         plot_data(smooth_time, smooth_values, param.replace('_data_', ""))
...
# Read and plot velocity data for each robot
plot_params = ['velocity_x1_data_', 'velocity_y1_data_', 'velocity_mag1_data_', 'accel_long1_data_', 'accel_lat1_data_', 'accel_mag1_data_']
for param in plot_params:
    plt.figure(figsize=(10, 5))  # Create a new figure for plots
    # plt.subplot(2,3,plot_params.index(param)+1)

    for namespace in robot_namespaces:
        time_bag_filename = '/home/balu/catkin_ws/src/move_robot/bags/'+'time1_data_' + namespace + '.bag'
        bag_filename = '/home/balu/catkin_ws/src/move_robot/bags/'+ param + namespace + '.bag'
        # print(bag_filename)
        time_data = read_bag_file(time_bag_filename, namespace+'_time1_data')
        # print(namespace+'_'+param)
        values = read_bag_file(bag_filename, namespace+'_'+param[:-1])
        # print(time_data)
        # print("flag 3",values)
